...
void TaskReception(void *param)
{
    ...
    while (1)
    {
        size_t random_task_id = esp_random() % (TaskFunction_NUM + ADDED_GARBAGE_REQUEST);

        Request_t new_req =
        {
            .idTask = random_task_id
        };
        ...
        else
        {
            ...
            printf("Error: TaskReception can't add request to queue\n");
            printf("New req: %zu", new_req.idTask);
        }
    }
